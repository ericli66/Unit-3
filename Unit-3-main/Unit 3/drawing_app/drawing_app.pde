...
void Interface() {
  ...
  // Indicator
  if (stampOn == true) {
    image(selectedStamp, 450, 75, stampSize, stampSize);
  } else {
...
void slider(int x, int y, int d) {
  ...
  fill(0);
  circle(sliderX, y, d);
  strokeSize = map(sliderX, 225, 375, 1, 20);
  stampSize = map(sliderX, 225, 375, 50, 150);
}

//to move the slider
void controlSlider() {
  if (dist(mouseX, mouseY, sliderX, 75) < 25/2) {
    sliderX = mouseX;
  }
...
void stampButton(PImage label, int x, int y, int w, int h) {
  if (selectedStamp == label && stampOn == true) {
    fill(white);
  } else if (mouseX > x && mouseX < x+w+15 && mouseY > y-5 && mouseY < y+h+5) {
    fill(white);
    if (mousePressed) {
      stampOn = true;
      selectedStamp = label;
...
void mouseDragged() {
  ...
  if (mouseY > 150) {
    if (stampOn == true) {
      image(selectedStamp, mouseX, mouseY, stampSize, stampSize);
    } else {
      strokeWeight(strokeSize);
      stroke(selectedColor);
